...
package soulwire.ai {
	...
	public class AwayBoid {
		...
		private var _screenCoords : Vector3D;
		private var _renderData : Entity;
		 ...
		 */
		public function get screenCoords() : Vector3D {
		 ...
		 */
		public function get renderData() : Entity {
			return _renderData;
		}

		public function setRenderData(value : Entity, _rad : Number) : void {
			_renderData = value;
			_radius = _rad;
		...
		// ___________________________________________________________
		// ——————————————————————————————————————————————— CONSTRUCTOR
		public function AwayBoid(maxForce : Number = 1.0, maxSpeed : Number = 10.0, edgeBehavior : String = EDGE_NONE) {
		...
		public function render() : void {
			if ( !_renderData || !_renderData.visible ) {
				return;
			}
			...
			if ( _lookAtTarget ) {
				...
				_renderData.lookAt(_velocity);
			}

			_matrix.appendTranslation(_position.x, _position.y, _position.z);

			/*
			_renderData.rotationX = _velocity
			_renderData.rotationY
			_renderData.rotationZ
			 */

			_renderData.x = _position.x;
			_renderData.y = _position.y;
			_renderData.z = _position.z;

			// _renderData.transform = _matrix;
		}
			...
			if ( _screenCoords.x < rect.left - _radius ) {
				switch( behavior ) {
					case EDGE_WRAP :
						...
						_renderData.transform.identity();
						_position.x = _renderData.x;
		 ...
		 * below 1.0
		 */
		public function flock(boids : Vector.<AwayBoid>, separationWeight : Number = 0.5, alignmentWeight : Number = 0.1, cohesionWeight : Number = 0.2, separationDistance : Number = 100.0, alignmentDistance : Number = 200.0, cohesionDistance : Number = 200.0) : void {
		 ...
		 * below 1.0
		 */
		public function separate(boids : Vector.<AwayBoid>, separationDistance : Number = 50.0, multiplier : Number = 1.0) : void {
		 ...
		 * below 1.0
		 */
		public function align(boids : Vector.<AwayBoid>, neighborDistance : Number = 40.0, multiplier : Number = 1.0) : void {
		 ...
		 * below 1.0
		 */
		public function cohesion(boids : Vector.<AwayBoid>, neighborDistance : Number = 10.0, multiplier : Number = 1.0) : void {
		...
		private function getSeparation(boids : Vector.<AwayBoid>, separation : Number = 25.0) : Vector3D {
		...
		private function getAlignment(boids : Vector.<AwayBoid>, neighborDistance : Number = 50.0) : Vector3D {
			var force : Vector3D = new Vector3D();
			var distance : Number;
			var count : int = 0;
			var boid : AwayBoid;
		...
		private function getCohesion(boids : Vector.<AwayBoid>, neighborDistance : Number = 50.0) : Vector3D {
			var force : Vector3D = new Vector3D();
			var distance : Number;
			var count : int = 0;
			var boid : AwayBoid;
		...
		private function calculateScreenCoords() : void {
			if ( !_position.equals(_oldPosition) ) {
				_screenCoords = _renderData.position;
			}
		}
