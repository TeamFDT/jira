...
package jiglib.geometry {
	...
	public class JAABox {
		...
		public function overlapTest(box : JAABox) : Boolean {
			return (
			(_minPos.z >= box.maxPos.z) || (_maxPos.z <= box.minPos.z) || (_minPos.y >= box.maxPos.y) || (_maxPos.y <= box.minPos.y) || (_minPos.x >= box.maxPos.x) || (_maxPos.x <= box.minPos.x) ) ? false : true;
		}

		public function isPointInside(pos : Vector3D) : Boolean {
			return ((pos.x >= _minPos.x) && (pos.x <= _maxPos.x) && (pos.y >= _minPos.y) && (pos.y <= _maxPos.y) && (pos.z >= _minPos.z) && (pos.z <= _maxPos.z));
		}
	}
}