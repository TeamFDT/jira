	...
	public class SWFLoader extends DisplayObjectLoader {
		...
		override protected function _load() : void {
			if (_stealthMode) {
				...
				// to avoid null object reference errors in code inside the child swf that may reference "stage" (we'll removeChild() as soon as it inits)
				super._load();
			} else if (_queue != null) {
		...
		override protected function _calculateProgress() : void {
			...
			if (_loader.contentLoaderInfo.bytesTotal != 0) {
				// otherwise if unload() was called, bytesTotal would go back down to 0.
		...
		protected function _init() : void {
			...
			if (!_scriptAccessDenied) {
				...
				if (_loader.parent == _sprite) {
					...
					// we only added it temporarily so that if the child swf references "stage" somewhere, it could avoid errors (as long as this SWFLoader's ContentDisplay is on the stage, like if a "container" is defined in vars)
				}
			} else {
				_content = _loader;
		...
		override protected function _failHandler(event : Event, dispatchError : Boolean = true) : void {
			if ((event.type == "ioError" || event.type == "securityError") && event.target == _loader.contentLoaderInfo) {
				...
				if (_loadOnExitStealth) {
					// could happen if the url is set to another value between the time the SWFLoader starts loading and when it fails.
