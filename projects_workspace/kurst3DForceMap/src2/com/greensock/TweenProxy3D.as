...
package com.greensock {
	...
	dynamic public class TweenProxy3D extends Proxy {
		...
		// populated with all TweenProxy3D instances with the same _target (basically a faster way to access _dict[_target])
		/** @private **/
		private var _localRegistration : Vector3D;
		// according to the local coordinates of _target (not _target.parent)
		/** @private **/
		private var _registration : Vector3D;
		// according to _target.parent coordinates
		/** @private **/
		private var _regAt0 : Boolean;
		// If the localRegistration point is at 0, 0, this is true. We just use it to speed up processing in getters/setters.
		/** @private **/
		...
		public function calibrate() : void {
			...
			_target.rotationX = _rotationX;
			// just forces the skew to be zero.
			_skewX = _skewY = 0;
		...
		public function set rotationX($n : Number) : void {
			...
			if (_skewX != 0 || _skewY != 0) {
				...
				if (_proxies.length > 1) {
					// if there are other proxies controlling the same _target, make sure their _registration variable is updated
		...
		private function updateWithSkew() : void {
			...
			if (_scaleX < 0) {
				// get around a bug in Flash which causes negative scaleX values to report as negative scaleY with 180 degrees added to the rotation!
				sx = -sx;
