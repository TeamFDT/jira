...
package com.greensock.layout.core {
	...
	public class LiquidData {
		...
		public function LiquidData(pin : PinPoint, target : DisplayObject, type : uint, liquidStage : LiquidStage, strict : Boolean, tweenDuration : Number, tweenVars : Object, roundPosition : Boolean) {
			...
			// LiquidStage.getByStage(this.target.stage);
			this.type = type;
		...
		public function refreshLevel() : void {
			...
			if (stage) {
				...
				// forces pins to be 1 level higher than the DisplayObjects which is necessary to have them refresh in the correct order.
			...
			if (this.type >= 2 && this.level < this.pin.data.level) {
				// if an object is pinned with a pin that's at a deeper nested level, we must let the pin update first, so adjust the level here.
