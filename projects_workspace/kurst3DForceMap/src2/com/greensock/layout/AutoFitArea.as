	...
	public class AutoFitArea extends Shape {
		...
		public static function createAround(target : DisplayObject, vars : Object = null, ...args) : AutoFitArea {
			if (vars == null || typeof(vars) == "string") {
				...
				vars = {scaleMode:vars || "proportionalInside", hAlign:args[0] || "center", vAlign:args[1] || "center", crop:Boolean(args[2]), minWidth:args[3] || 0, maxWidth:(isNaN(args[4]) ? 999999999 : args[4]), minHeight:args[5] || 0, maxHeight:(isNaN(args[6]) ? 999999999 : args[6]), calculateVisible:Boolean(args[8])};
			}
		...
		public function attach(target : DisplayObject, vars : Object = null, ...args) : void {
			...
			if (vars == null || typeof(vars) == "string") {
				// sensed old method - parse the params for backwards compatibility
				vars = {scaleMode:vars || "proportionalInside", hAlign:args[0] || "center", vAlign:args[1] || "center", crop:Boolean(args[2]), minWidth:args[3] || 0, maxWidth:(isNaN(args[4]) ? 999999999 : args[4]), minHeight:args[5] || 0, maxHeight:(isNaN(args[6]) ? 999999999 : args[6]), calculateVisible:Boolean(args[7]), customAspectRatio:Number(args[8]), roundPosition:Boolean(args[9])};
			}
...
internal class AutoFitItem {
	...
	public function AutoFitItem(target : DisplayObject, vars : Object, next : AutoFitItem) {
		...
		if (this.hasDrawNow) {
			...
			// just to make sure we're starting with the correct values if it's a component.
		}
