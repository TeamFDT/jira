	...
	public class PhysicsPropsPlugin extends TweenPlugin {
		...
		override public function onInitTween(target : Object, value : *, tween : TweenLite) : Boolean {
			...
			if (tl == TweenLite.rootFramesTimeline) {
				// indicates the tween uses frames instead of seconds.
				_stepsPerTimeUnit = 1;
			}
			_props = [];
		...
		override public function set changeFactor(n : Number) : void {
			...
			if (!this.round) {
				...
				while (i--) {
					val = values[i];
					if (val > 0) {
						_target[PhysicsProp(_props[i]).property] = (val + 0.5) >> 0;
...
internal class PhysicsProp {
	...
	public var v : Number;
	// used to track the current velocity as we iterate through friction-based tween algorithms
	public var a : Number;
	// only used in friction-based tweens
	public var value : Number;

	// used to track the current property value in friction-based tweens
