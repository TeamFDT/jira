		...
		override public function onInitTween(target : Object, value : *, tween : TweenLite) : Boolean {
			...
			if ((!isNaN(value.width) || !isNaN(value.height)) && _target.parent != null) {
				...
				if (point.x == _target.parent.globalToLocal(_target.localToGlobal(new Point(100, 100))).x) {
					// checks to see if the width change also alters where the 100,100 point is in the parent, essentially telling us whether or not the width change also effectively changed the scale, but we can't just check the scaleX because rotation would affect it and there are some inconsistencies in the way Adobe's classes/components work.
					_proxy = _target;
					_target.rotation = 0;
					_proxySizeData = {};
					if (!isNaN(value.width)) {
						addTween(_proxySizeData, "width", _target.width / 2, value.width, "width");
						// Components that alter their width without scaling will treat their width/height setters as though they were applied without any rotation, so we must handle these separately. If we just allow the width/height tweens to affect the Sprite and copy those values over to the _proxy, it won't behave properly.
					}
					...
					var container : Sprite = _isFlex ? new (getDefinitionByName("mx.core.UIComponent"))() : new Sprite();
					// in Flex, any thing we addChild() must be a UIComponent so we wrap our Sprite in one.
					...
					s.graphics.endFill();
					_proxy.width /= 2;
					// we must reset the width even though we're applying the transform.matrix after this because some components don't flow the transform.matrix values through to the width value (bug/inconsistency in Adobe's stuff)
