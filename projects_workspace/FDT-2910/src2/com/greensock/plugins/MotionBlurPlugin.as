...
package com.greensock.plugins {
	...
	public class MotionBlurPlugin extends TweenPlugin {
		...
		// we keep recording this value every time the _target moves at least 2 pixels in either direction in order to accurately determine the angle (small measurements don't produce accurate results).
		/** @private **/
		...
		public function MotionBlurPlugin() {
			...
			this.priority = -2;
			// so that the x/y/alpha tweens occur BEFORE the motion blur is applied (we need to determine the angle at which it moved first)
		...
		override public function onInitTween(target : Object, value : *, tween : TweenLite) : Boolean {
			...
			if (!_classInitted) {
				try {
					...
					// SystemManager is the first display class created within a Flex application
				} catch (e : Error) {
					_isFlex = false;
				}
				_classInitted = true;
		...
		private function disable() : void {
			...
			if (_sprite.parent) {
				if (_isFlex && _sprite.parent.hasOwnProperty("removeElement")) {
					// to accommodate Flex 4 API additions
		...
		private function onTweenDisable() : void {
			if (_tween.cachedTime != _tween.cachedDuration && _tween.cachedTime != 0) {
				// if the tween is on a TimelineLite/Max that eventually completes, another tween might have affected the target's alpha in which case we don't want to mess with it - only disable() if it's mid-tween. Also remember that from() tweens will complete at a value of 0, not 1.
				disable();
			}
		}
		...
		override public function set changeFactor(n : Number) : void {
			...
			if (time < 0) {
				time = -time;
				// faster than Math.abs(_tween.cachedTime - _time)
			}

			if (time < 0.0000001) {
				return;
				// number is too small - floating point errors will cause it to render incorrectly
			}
			...
			if (!_fastMode || !_cached) {
				...
				if (parentFilters.length != 0) {
					...
					// if the parent has filters, it will choke when we move the child object (_target) to x/y of 20,000/20,000.
				}

				_target.x = _target.y = 20000;
				// get it away from everything else;
				...
				if (_parentMask) {
					...
					// otherwise the mask could interfere with draw()
				}
			...
			// use 0.00390625 instead of 0 so that we can identify if it was changed outside of this plugin next time through. We were running into trouble with tweens of alpha to 0 not being able to make the final value because of the conditional logic in this plugin.
