...
package com.greensock.layout {
	...
	public class LiquidStage extends EventDispatcher {
		...
		public function attach(target : DisplayObject, pin : PinPoint, strict : Boolean = false, reconcile : Boolean = true, tweenDuration : Number = 0, tweenVars : Object = null, roundPosition : Boolean = false) : void {
			...
			this.release(target);
			// in case it's already being watched or was pinned with another PinPoint.
		...
		private function updateItems() : void {
			...
			while (--i > -1) {
				item = _items[i];
				if (item.type == 3) {
					(item.target as Object).capturePinData();
					// didn't strict data type this so that file size could be minimized (otherwise LiquidArea and AutoFitArea would be forced to be included)
			...
			while (--i > -1) {
				...
				if (item.type == 3) {
					(item.target as Object).updatePins();
					// didn't strict data type this so that file size could be minimized (otherwise LiquidArea and AutoFitArea would be forced to be included)
		...
		public function refreshLevels() : void {
			// nesting levels are extremely important in terms of the order in which we update LiquidItems and PinPoints...
			var i : int = _items.length;
			while (--i > -1) {
				LiquidData(_items[i]).refreshLevel();
			}
			_items.sortOn("level", Array.NUMERIC | Array.DESCENDING);
			// to ensure that nested objects are updated AFTER their parents.
		}
		...
		override public function addEventListener(type : String, listener : Function, useCapture : Boolean = false, priority : int = 0, useWeakReference : Boolean = false) : void {
			_hasListener = true;
			// just so we can avoid dispatching events when there are no listeners (which will likely happen most of the time)
