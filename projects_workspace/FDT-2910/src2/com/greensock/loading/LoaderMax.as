	...
	public class LoaderMax extends LoaderCore {
		...
		public function insert(loader : LoaderCore, index : uint = 999999999) : LoaderCore {
			...
			if (this != _globalRootLoader) {
				...
				// use -1 so that if the user adds an event listener, it gets called before LoaderMax is notified. Otherwise bubbling behavior doesn't go in the proper order.
			...
			if (this.autoLoad && loader.status == LoaderStatus.READY) {
				if (_status != LoaderStatus.LOADING) {
					...
					_loadNext(null);
					// to ensure the maxConnections pipeline is full
				}
			}
		...
		protected function _removeLoader(loader : LoaderCore, rootLoaderAppend : Boolean) : void {
			...
			// has conditional logic that will only dispatch a PROGRESS event if bytesLoaded or bytesTotal has changed.
		}
		...
		override protected function _progressHandler(event : Event) : void {
			...
			if (_dispatchProgress && _status != LoaderStatus.DISPOSED) {
				...
				if (bl == 0 && _cachedBytesLoaded == 0) {
					// do nothing
				} else if ((_cachedBytesLoaded != _cachedBytesTotal || _status != LoaderStatus.LOADING) && (bl != _cachedBytesLoaded || bt != _cachedBytesTotal)) {
