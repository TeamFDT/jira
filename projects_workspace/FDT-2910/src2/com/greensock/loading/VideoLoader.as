...
package com.greensock.loading {
	...
	public class VideoLoader extends LoaderItem {
		...
		public function VideoLoader(urlOrRequest : *, vars : Object = null) {
			...
			// so that the video doesn't initially show at the wrong size before the metaData is received at which point we can accurately determine the aspect ratio.
		}
		...
		override protected function _calculateProgress() : void {
			...
			if (_cachedBytesLoaded > 1) {
				...
				if (_cachedBytesTotal <= _cachedBytesLoaded) {
					_cachedBytesTotal = ((this.metaData != null && _renderedOnce && _initted) || (getTimer() - _time >= 10000)) ? _cachedBytesLoaded : int(1.01 * _cachedBytesLoaded) + 1;
		...
		public function gotoVideoTime(time : Number, forcePlay : Boolean = false, skipCuePoints : Boolean = true) : Number {
			...
			if (_initted && _renderedOnce && changed) {
				// don't seek() until metaData has been received otherwise it can prevent it from ever being received. Also, if the NetStream hasn't rendered once and we seek(), it often completely loses its audio!
				_seek(time);
			} else {
				_setForceTime(time);
			}
		...
		protected function _applyPendingPause() : void {
			...
			if (!_autoDetachNetStream || _video.stage != null) {
				...
				// works around an odd bug in Flash that can cause the video not to render when it is attached and paused immediately.
				_video.attachNetStream(_ns);
				// in case it was removed
				_ns.pause();
				// If we pause() the NetStream when it isn't attached to the _video, a bug in Flash causes it to act like it continues playing!!!
			}
		}
		...
		protected function _forceInit() : void {
			...
			Object(_sprite).rawContent = _video;
		...
		protected function _metaDataHandler(info : Object) : void {
			if (this.metaData == null || this.metaData.cuePoints == null) {
				// sometimes videos will trigger the onMetaData multiple times (especially F4V files) and occassionally the last call doesn't contain cue point data!
				this.metaData = info;
			}
			...
			if (!_initted) {
				...
				// on rare occasions, _metaDataHandler() is called twice by the NeStream (particularly for F4V files) and the 2nd call contains more data than the first, so just in case the width/height changed, we set the rawContent of the ContentDisplay to make sure things render according to the correct size.
			}
		}
		...
		protected function _cuePointHandler(info : Object) : void {
			if (!_videoPaused) {
				// in case there's a cue point very early on and autoPlay was set to false - remember, to work around bugs in NetStream, we cannot pause() it until we receive metaData and the first frame renders.
		...
		protected function _playProgressHandler(event : Event) : void {
			...
			if (_bufferFull && (_firstCuePoint || _dispatchPlayProgress)) {
				...
				_prevTime = ((_forceTime || _forceTime == 0) && _ns.time <= _duration) ? _ns.time : this.videoTime;
				// note: isNaN(_forceTime) is much slower than !(_forceTime || _forceTime == 0)
				var next : CuePoint;
				var cp : CuePoint = _firstCuePoint;
				while (cp) {
					next = cp.next;
		...
		protected function _statusHandler(event : NetStatusEvent) : void {
			...
			if (code == "NetStream.Play.Stop") {
				...
				if (!_autoDetachNetStream && !isNaN(_forceTime)) {
					_renderHandler(null);
					// note: do not _ns.pause() here when the NetStream isn't attached to the _video because a bug in Flash will prevent it from working (just when this NetStreamEvent occurs!)
				}
				...
				var revisedBufferTime : Number = videoRemaining * (1 - (videoRemaining / loadRemaining)) * 0.9;
		...
		override public function auditSize() : void {
			if (_url.substr(0, 4) == "http" && _url.indexOf("://") != -1) {
				// if the url isn't relative, use the regular URLStream to do the audit because it's faster/more efficient.
				super.auditSize();
		...
		protected function _renderHandler(event : Event) : void {
			...
			if (!_videoPaused || _initted) {
				// if the video hasn't initted yet and it's paused, keep reporting the _forceTime and let the _renderTimer keep calling until the condition is no longer met.
				_forceTime = NaN;
				_renderTimer.stop();
				_ns.removeEventListener(Event.RENDER, _renderHandler);
			}
		...
		protected function _videoAddedToStage(event : Event) : void {
			if (_autoDetachNetStream) {
				if (!_pausePending) {
					...
					// a bug in Flash prevents the video from rendering visually unless we seek() when we attachNetStream()
				}
				_video.attachNetStream(_ns);
			}
		}
