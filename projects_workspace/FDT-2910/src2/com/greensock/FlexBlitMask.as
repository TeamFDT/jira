	...
	public class FlexBlitMask extends UIComponent {
		...
		protected function _captureTargetBitmap() : void {
			if (_bd == null || _target == null) {
				// must have been disposed, so don't update.
				return;
			}
		...
		public function update(event : Event = null, forceRecaptureBitmap : Boolean = false) : void {
			if (_bd == null) {
				...
				if (this.parent != _target.parent) {
					if (_target.parent.hasOwnProperty("addElementAt")) {
						// for Flex compatibility (spark)
		...
		protected function _render(xOffset : Number = 0, yOffset : Number = 0, clear : Boolean = true, limitRecursion : Boolean = false) : void {
			...
			if (_bd == null) {
				...
				// sometimes (especially in Flex) objects take a frame or two to render in Flash and properly report their width/height. Before that, their width/height is typically 0. This works around that issue and forces a refresh if we didn't capture any pixels last time we did a capture.
				_captureTargetBitmap();
			}
			...
			if (clear) {
				...
				// subtract 1 to compensate for the pixel we added above.
			...
			if (_bitmapMode != value) {
				...
				if (_target != null) {
					...
					if (_bitmapMode) {
						...
						// if cacheAsBitmap is true on both the _target and the FlexBlitMask instance, the transparent areas of the mask will be...well...transparent which isn't what we want when bitmapMode is false (it could hide visible areas unless update(null, true) is called regularly, like if the target has animated children and bitmapMode is false)
						_target.mask = this;
						if (_wrap) {
							normalizePosition();
						}
		...
		public function set target(value : DisplayObject) : void {
			if (_target != value) {
				...
				if (_target != null) {
					...
					// forces a refresh (applying the mask, doing an update(), etc.)
				} else {
					_bounds = new Rectangle();
				}
			}
